...
import {
  ...
  type DuasLandingContent,
} from "@/lib/duas-landing-content";

const s = (v: unknown, fb = "") => (typeof v === "string" ? v : fb);

function sanitize(input: DuasLandingContent): DuasLandingContent {
...
/** Persists the /duas content (admin-guarded). */
export async function updateDuasLandingContent(input: DuasLandingContent) {
  ...
  try {
    await db
      .insert(appSetting)
      .values({ key: DUAS_LANDING_KEY, value: JSON.stringify(clean), updatedAt: new Date() })
      .onConflictDoUpdate({
        target: appSetting.key,
        set: { value: JSON.stringify(clean), updatedAt: new Date() },
      });
  } catch (e: any) {
    console.error("[duas-landing] update failed:", e);

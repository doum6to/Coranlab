...
import {
  CORAN_PRIERE_LANDING_KEY,
  CORAN_PRIERE_LANDING_DEFAULTS,
} from "@/lib/coran-priere-landing-content";

const s = (v: unknown, fb = "") => (typeof v === "string" ? v : fb);

function sanitize(input: CoranLandingContent): CoranLandingContent {
  const d = CORAN_PRIERE_LANDING_DEFAULTS;
...
/** Persists the /comprendre-sa-priere content (admin-guarded). */
export async function updateCoranPriereLandingContent(input: CoranLandingContent) {
  ...
  try {
    ...
    console.error("[coran-priere-landing] update failed:", e);

...
export function DuasOrdersForm({ initial }: { initial: DriveOrderRow[] }) {
  ...
  const run = (
    ...
    startTransition(async () => {
      ...
      if (res?.error) setMsg(res.error);
        ...
        setMsg("Validée, mais l'email n'est pas parti (voir logs Resend).");
      setBusyId(null);
      router.refresh();
    });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-neutral-600">
          Commandes /duas (carte + Orange Money) ·{" "}
              ...
              {initial.map((o) => (
                <tr key={o.id} className="align-middle">
                  ...
                  <td className="whitespace-nowrap px-3 py-2 text-xs text-neutral-600">
                    {o.source === "card" ? "Carte" : "Orange Money"}
                  </td>
                    ...
                    {o.status === "pending" ? (
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          ...
                          onClick={() => run(o.id, () => approveDriveOrder(o.id))}
                        ...
                        <button
                          disabled={pending && busyId === o.id}
                          onClick={() => run(o.id, () => rejectDriveOrder(o.id))}
